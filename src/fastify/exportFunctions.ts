...
export function generate_book(genres: string[], id: number = Math.round(Math.random() * 1000)): string { 
    let res: Book = {
        "id": id,
...
export function delete_book(id: number): string {
  const data = fs.readFileSync("dist/data/books.json", "utf-8");
...
export function delete_author(id: number): string {
  const data = fs.readFileSync("dist/data/authors.json", "utf-8");
  const authors = JSON.parse(data);

  const updatedAuthors = authors.filter((author: any) => author.id !== id);
...
export function delete_publisher(id: number): string {
  const data = fs.readFileSync("dist/data/publishers.json", "utf-8");
  const publishers = JSON.parse(data);

  const updatedPublishers = publishers.filter((publisher: any) => publisher.id !== id);
  ...
  fs.writeFileSync(
    "dist/data/publishers.json",
    JSON.stringify(updatedPublishers, null, 2)
  );
...
export function generate_review(idList :number[], bookId :number = idList[Math.floor(Math.random() * idList.length)]) {
    let res: Review = {
        id: Math.round(Math.random() * 1000),
...
export function delete_review(id: number): string {
  const data = fs.readFileSync("dist/data/reviews.json", "utf-8");
...
export function getReviewByBookId(id: number) {
    const data = fs.readFileSync("dist/data/reviews.json", "utf-8");
    const jsonData = JSON.parse(data);
...
export function review_cleaner(idList: number[]) :void{
    const data = fs.readFileSync("dist/data/reviews.json", "utf-8");
    const reviews = JSON.parse(data);
    for(let i = 0; i < reviews.length; i++){
        if(!(reviews[i].bookId in idList)){
            const updatedReviews = reviews.filter((review: any) => review.bookId !== reviews[i].bookId);
            fs.writeFileSync(
            "dist/data/reviews.json",
            JSON.stringify(updatedReviews, null, 2));
        }
...
export function getBookRating(bookId: number){
    const data = fs.readFileSync("dist/data/reviews.json", "utf-8");
    const reviews = JSON.parse(data);
